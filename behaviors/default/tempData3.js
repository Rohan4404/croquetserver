...
//         // Fetch the stored object name from the API
//         const storedData = await fetchStoredObjectName();

//         // Find the object in the scene with the name matching the stored data
//         // const storedObject = allowedObjects.find(obj => obj.name === storedData);

//         // Highlight the stored object and display its information
//         // if (storedObject) {
//         //   console.log("stored object is 1")
//         //   highlightObject(storedObject); // Highlight stored object instead of clicked object
//         //   handleObjectInteraction(storedObject); // Display relevant info
//         // }
//       } else {
//         resetHighlightAndHideInfo();
//         console.log("stored object is 2");
//         if (highlightedObject) {
//           console.log("stored object is 3");
...
//       // Set a highlight material (yellow, for example)
//       const highlightMaterial = new THREE.MeshBasicMaterial({
//         color: 0xffff00,
//       });
//       object.material = highlightMaterial;

//       // Mark the object as the highlighted object
//       highlightedObject = object;
//     };

//     const resetObjectMaterial = (object) => {
//       if (originalMaterials.has(object)) {
//         // Restore the object's original material
//         object.material = originalMaterials.get(object);
//         originalMaterials.delete(object); // Optional: Remove the stored material
//       }
//     };
...
//       console.log("allow object is ", allowedObjects);
//       const objectToHighlight = Array.from(allowedObjects).find(
//         (obj) => obj.name === name
//       );

//       console.log(objectToHighlight, "objectToHighlight is ");
...
//       object.traverse((child) => {
//         if (child.isMesh) {
//           child.material.color.set(child.userData.originalColor); // Reset to original color
//         }
//       });
//     };
  ...
  setup() {
    ...
    const onDocumentMouseClick = async (event) => {
      ...
      if (intersects.length > 0) {
        ...
        // Fetch the API value (toggle the API)
        const apiValue = await toggleAPI();
        ...
        if (apiValue === 1) {
          // Store clicked object's name in the API
          await storeClickedObjectName(clickedObject.name);
          highlightObject(clickedObject); // Highlight the clicked object
          handleObjectInteraction(clickedObject); // Handle object interaction (show details, etc.)
        } else {
          resetHighlightAndHideInfo();
          await storeClickedObjectName("NULL");
        }

        // Optionally fetch the stored object name from the API
        const storedData = await fetchStoredObjectName();
    ...
    const toggleAPI = async () => {
      try {
        ...
        const result = await response.json();
        console.log(`API Response: ${result}`);
        return result; // Assuming the API returns the new state value (1 or 0)
      } catch (error) {
        console.error("Error toggling API:", error);
        return 0; // Return 0 in case of an error
      }
    ...
    loadModelPromise
      .then((model) => {
        ...
        if (!window.gui) {
          ...
          params = {
            temperatureThreshold: 35, // Default temperature threshold
            checkTemperature: false, // Initially unchecked
          };
