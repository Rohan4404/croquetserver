...
//         // Handle based on the API value
//         if (apiValue === 1) {
//           // Store clicked object's name in the API
//           await storeClickedObjectName(clickedObject.name);
//         } else {
//           // Reset everything if API value is 0
//           resetHighlightAndHideInfo();
//           await storeClickedObjectName("NULL");
//           return; // Exit early to avoid further processing
//         }

//         // Fetch the stored object name
//         const storedData = await fetchStoredObjectName();

//         // Highlight the clicked object or reset if it's already highlighted
...
//           // Now handle the highlighting and showing information based on the clicked object
//           highlightObject(clickedObject); // Highlight the actual clicked object
//           speakObjectName(clickedObject.name); // Speak the clicked object's name

//           // Check if the clicked object is a rack, tile, server, or other object
//           let isTile = false;
//           let isRack = false;
//           let isObject = false;
...
//           } else {
//             // Check if clicked object is a rack
//             for (let i = 0; i <= 70; i++) {
//               if (clickedObject === model.children[i].children[0]) {
//                 if (i >= 5 && i <= 15) continue; // Skip certain racks
//                 isRack = true;
//                 break;
//               }
//             }
...
//               hideCharts4();
//             } else {
//               // Check if clicked object is another specific object type
//               for (let i = 0; i <= 17; i++) {
//                 for (let j = 0; j <= 2; j++) {
//                   if (
...
//                 hideCharts2();
//               } else {
//                 // Default: treat as a server and display server info
//                 displayServerInfo(clickedObject);
//                 hideHVACInfo();
...
//         }
//       } else {
//         // If no object is clicked, reset everything
//         resetHighlightAndHideInfo();
...
//     // const highlightObject = (object) => {
//     //   // Ensure the object has a material before attempting to clone it
...
//     //     highlightedObject = object;
//     //   } else {
//     //     console.warn("Object does not have a material property:", object);
//     //   }
//     // };
...
//     // // Function to reset highlight and hide all info
//     // const resetObjectMaterial = (object) => {
...
//       if (highlightedObject) {
//         resetObjectMaterial(highlightedObject); // Reset material first
//         stopSpeaking();
//         highlightedObject = null;
//         hideAllInfo(); // Then hide info
//       }
//     };
...
class LightPawn extends PawnBehavior {
  setup() {
    ...
    let model;
    const originalMaterials = new Map(); // Store original materials for each object
    const highlightedObjects = new Map(); // Store highlighted objects per user
    const allowedObjectsPerUser = new Map(); // Store allowed objects per user
    const allowedObjects = new Set();

    const onDocumentMouseClick = async (event, userId) => {
      const allowedObjects = allowedObjectsPerUser.get(userId);
      const mouse = new THREE.Vector2(
        (event.clientX / window.innerWidth) * 2 - 1,
        -(event.clientY / window.innerHeight) * 2 + 1
      );
      ...
      if (intersects.length > 0) {
        ...
        if (apiValue === 1) {
          await storeClickedObjectName(clickedObject.name, userId);

          if (highlightedObjects.get(userId) === clickedObject) {
            resetHighlightAndHideInfo(userId);
          } else {
            if (highlightedObjects.get(userId)) {
              resetObjectMaterial(highlightedObjects.get(userId));
            }

            highlightObject(clickedObject);
            speakObjectName(clickedObject.name);
            highlightedObjects.set(userId, clickedObject);
          }
        } else {
          resetHighlightAndHideInfo(userId);
          await storeClickedObjectName("NULL", userId);
        }
      }
    };
    ...
    const resetHighlightAndHideInfo = (userId) => {
      const highlightedObject = highlightedObjects.get(userId);
      if (highlightedObject) {
        resetObjectMaterial(highlightedObject); // Reset material first
        stopSpeaking();
        highlightedObjects.delete(userId);
        hideAllInfo(); // Then hide info
      }
    };

    const storeClickedObjectName = async (name, userId) => {
      try {
        const response = await fetch(
          `https://termpvariable.vercel.app/store-data/${userId}`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
        ...
        const result = await response.json();
        console.log("Stored object name in API for user:", result);
      } catch (error) {
        console.error("Error storing clicked object name in API:", error);
      }
    };
